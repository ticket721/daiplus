...
module.exports = {
    mtransferfrom_unwrap_transfer: async function mtransferfrom_unwrap_transfer(accounts, expect) {
        ...
        await devdai_instance.test__mint(accounts[0], signer_amount + relayer_amount, {from: accounts[0]});
        await devdai_instance.approve(daiplus_instance.address, signer_amount + relayer_amount, {from: accounts[0]});

        expect((await devdai_instance.balanceOf(wallet.address)).toNumber()).to.equal(0);
        expect((await devdai_instance.balanceOf(accounts[0])).toNumber()).to.equal(signer_amount + relayer_amount);
        expect((await devdai_instance.balanceOf(accounts[2])).toNumber()).to.equal(0);
        expect((await devdai_instance.balanceOf(daiplus_instance.address)).toNumber()).to.equal(0);
        ...
        await daiplus_instance.wrap(wallet.address, signer_amount + relayer_amount, {from: accounts[0]});
        ...
        expect((await daiplus_instance.balanceOf(wallet.address)).toNumber()).to.equal(signer_amount + relayer_amount);
        expect((await daiplus_instance.balanceOf(accounts[0])).toNumber()).to.equal(0);
        expect((await daiplus_instance.balanceOf(accounts[2])).toNumber()).to.equal(0);
        expect((await daiplus_instance.balanceOf(daiplus_instance.address)).toNumber()).to.equal(0);
        ...
        await display_state('BEFORE signedTransferFrom META-TRANSACTION',
            [
                ...
                [accounts[2], 'Sender (unwrap recipient)'],
                [daiplus_instance.address, 'Recipient'],
                [accounts[0], 'Relayer']
        ...
        expect((await devdai_instance.balanceOf(wallet.address)).toNumber()).to.equal(0);
        expect((await devdai_instance.balanceOf(accounts[0])).toNumber()).to.equal(0);
        expect((await devdai_instance.balanceOf(accounts[2])).toNumber()).to.equal(relayer_amount);
        expect((await devdai_instance.balanceOf(daiplus_instance.address)).toNumber()).to.equal(signer_amount);

        expect((await daiplus_instance.balanceOf(wallet.address)).toNumber()).to.equal(0);
        expect((await daiplus_instance.balanceOf(accounts[0])).toNumber()).to.equal(signer_amount);
        expect((await daiplus_instance.balanceOf(accounts[2])).toNumber()).to.equal(0);
        expect((await daiplus_instance.balanceOf(daiplus_instance.address)).toNumber()).to.equal(0);
