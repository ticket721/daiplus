...
const {display_state, domain} = require('./utils');

module.exports = {
    mtransfer_specific_relayer: async function mtransfer_specific_relayer(accounts, expect) {
        ...
        await display_state('AFTER signedTransfer META-TRANSACTION',
            [
                [wallet.address, 'Signer'],
                [accounts[0], 'Recipient'],
                [accounts[1], 'Relayer']
        ...
        expect((await daiplus_instance.balanceOf(wallet.address)).toNumber()).to.equal(amount - (100000 + 1234));
        expect((await daiplus_instance.balanceOf(to)).toNumber()).to.equal(1234);
