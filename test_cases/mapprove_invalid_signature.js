...
const {ZERO, ZEROSIG, display_state, domain} = require('./utils');

module.exports = {
    mapprove_invalid_signature: async function mapprove_invalid_signature(accounts, expect) {
        ...
        await expect(daiplus_instance.verifyApprove(
            [
            ...
            ZEROSIG
            , {from: accounts[1], gasPrice})).to.eventually.be.rejectedWith('DaiPlus::verifyApprove | invalid signer');

        // Execute Meta Transaction
        await expect(daiplus_instance.signedApprove(
            [
                mApprovePayload.actors.signer,
                mApprovePayload.actors.relayer,
                mApprovePayload.spender
            ...
            ZEROSIG
            , {from: accounts[1], gasPrice})).to.eventually.be.rejectedWith('DaiPlus::_signedApprove | invalid signer')
